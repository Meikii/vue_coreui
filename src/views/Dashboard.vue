...
export default {
  ...
  mounted(){
     if(window.localStorage.api_key == undefined)
      window.location.replace('http://localhost/#/pages/login')
    http://demo.satrapp.com/issues.json
    Axios.get('http://demo.satrapp.com/projects.json',{
      headers: {
    ...
    Axios.get('http://demo.satrapp.com/issues.json',{
      headers: {
              'Content-Type': 'application/json',
              'X-Redmine-API-Key': window.localStorage.api_key
      }
